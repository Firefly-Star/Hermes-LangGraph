...
class TestUtils:
    ...
    def test_ensure_dir_creates(self):
        _clean_agent_pool()
        assert not os.path.exists(ap.POOL_DIR)
        ap._ensure_dir()
        assert os.path.isdir(ap.POOL_DIR)

    def test_read_json_empty(self):
        ap._ensure_dir()
        data = ap._read_json(ap.REGISTRY_PATH)
        assert data == {}
    ...
    def test_read_write_json_roundtrip(self):
        ...
        ap._write_json(ap.REGISTRY_PATH, original)
        loaded = ap._read_json(ap.REGISTRY_PATH)
    ...
    def test_write_json_creates_dir(self):
        ...
        assert not os.path.exists(ap.POOL_DIR)
        data = {"k": "v"}
        ap._write_json(ap.CONTEXT_PATH, data)
        assert os.path.isdir(ap.POOL_DIR)
        assert os.path.exists(ap.CONTEXT_PATH)

    def test_append_jsonl(self):
        ap._ensure_dir()
        ap._append_jsonl(ap.CALLS_LOG_PATH, {"seq": 1, "msg": "hello"})
        ap._append_jsonl(ap.CALLS_LOG_PATH, {"seq": 2, "msg": "world"})
        with open(ap.CALLS_LOG_PATH, "r", encoding="utf-8") as f:
...
class TestContextManager:
    ...
    def test_persistence(self):
        ap._ensure_dir()
        ctx1 = ap.ContextManager()
        ctx1.set_bg("stored_key", "stored_value")
        ctx2 = ap.ContextManager()
...
class TestAgentManager:
    ...
    def test_create_agent_writes_to_file(self):
        ...
        raw = ap._read_json(ap.REGISTRY_PATH)
...
class TestConversationManager:
    def test_call_agent_not_found(self):
        ap._ensure_dir()
        cm = ap.ConversationManager(ap.AgentManager(), ap.Logger(), ap.Config())
    ...
    def test_call_gateway_not_running(self):
        ...
        cm = ap.ConversationManager(mgr, ap.Logger(), ap.Config())
