...
src/workflow/
├── __init__.py       # 包标记
├── __main__.py       # 入口：python -m src.workflow
...
```
ResumeRouter → [Phase 0: 澄清] → Flush → [Phase 1: PM] → Flush → [Phase 2: Dev] → Flush → [Phase 3: QA] → Flush → [Phase 4: 交付] → END
```
...
- **Phase 3**：QA 全流程：审核标准 → 测试计划 → 测试代码 → 运行 → 修 bug 循环
- **Phase 4**：一致性审计 → 维护文档 → 交付总结
