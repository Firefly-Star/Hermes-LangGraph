...
class DevExecStep:
    ...
    def write_step_letter(state) -> dict:
        ...
        write_letter(runtime, "master", master_conv, lpath,
                     ...
                     + "\n"
                     f"完成实现后自行运行验收方法确认通过。\n"
                     f"5. 在 {dev_dir} 目录下创建 .runner.md，写明如何运行当前步骤的测试，"
                     f"格式如下：\n"
                     "   ```markdown\n"
                     "   # Test Runner\n"
                     "   - 依赖：列出额外依赖\n"
                     ...
                     "   - 清理：测试后的清理步骤\n"
                     "   ```\n"
                     "   目的是让 Reviewer 能根据 .runner.md 复现测试环境。\n\n"
                     "## 注意\n"
    ...
    def read_step_letter(state) -> dict:
        ...
        read_letter(runtime, "dev", dev_conv, lpath,
                    ...
                    + "\n".join("   " + l for l in PLAYWRIGHT_TEST_TIPS.strip().split("\n"))
                    + "\n\n"
                    "5. 实现完成后，在 Dev/ 目录下创建 .runner.md，"
                    "写明如何运行当前步骤的测试（依赖、启动、运行命令、清理），"
                    "方便 Reviewer 复现测试环境。\n\n"
                    "## 参考文件\n"
...
class DevReviewStep:
    ...
    def run(state) -> dict:
        ...
        review = call_agent(runtime, "reviewer", conv_name(f"review-step-{step_idx + 1}"),
            ...
            "### 第二步：运行测试\n"
            "如果测试代码检查通过，先查看 Dev/ 目录下的 .runner.md（如果有），"
            "按其中说明配置环境并运行测试：\n"
            "1. 阅读 .runner.md 了解测试的依赖、启动方式和运行命令\n"
            "2. 执行 plan 中定义的验收命令\n"
            "3. 确认测试全部通过，无报错\n"
            "4. 如果有测试失败，诊断是代码问题还是测试问题\n\n"
            "### 第三步：代码审查\n"
            "1. 代码是否与详细设计方案一致\n"
            "2. 代码质量和错误处理是否合理\n"
